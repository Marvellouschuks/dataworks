(function(){
  ...
  let path=location.pathname, DOMLoaded, slice=(arg, ...n)=>[].slice.call(arg, ...n), qS=(el,str,all,res)=>(typeof str==='boolean'&&(all=str), typeof el=='string'&&(str=el,el=0), res=(el||document)['querySelector'+'All'.repeat(all)](str),all?slice(res):res);
  window.addEventListener('DOMContentLoaded', _=>{
      ...
      img.src='/wp-content/themes/dataworks/images/DataWorks.svg', img.onload=(el, news)=>{
        ...
        (window.onresize=_=>loop(itrvl=>{
          path==='/'&&qS('button.c02124',!0)[3]?.classList.add('hidden'), ['[data-stage="3"]', '.c02115']
    ...
    loop((el, toText)=>{
      toText=(index, clear)=>{
        if(!el) return;
        let lines=qS(el,'.line',!0),
        txt=(line, flag)=>{ if(!line) return; (line=line.firstElementChild).classList.add('duration-700', 'ease-out'), line.style.transform=`translate(0px, ${flag?9:''}0%)` },
        texts=txts[index].split(/\n/), once, asTxt=cb=>texts.forEach((text,i)=>lines[i].firstElementChild.textContent=(cb&&cb(lines[i]), text));
        if(clear) return asTxt();/**fires once when navigating to the last section */
      ...
      }, /about/.test(path)?done||=!about():done=0,
      path==='/'&&(/built for/i.test((el=qS('.c02132'))?.textContent) ? (reset&&toText(0,!0),/**reset after navigating to section*/btns=qS(el,'button',!0),
        el.onclick=(ev,btn)=>(btn=ev.target).tagName=='BUTTON'&&toText(btns.indexOf(btn)), window.atPageEnd=!(reset=0))
        : window.atPageEnd=!(reset=!0))
    });
    function about(ch){
